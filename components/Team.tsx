    ...
    <section id="team" className="px-3 py-12 md:px-5 md:py-16">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <a
            href="#contact"
            className="inline-flex items-center gap-2 self-start rounded-full bg-ink px-5 py-2 text-sm font-medium text-white"
          >
            Get in touch
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {team.map((member) => (
            <article
              key={member.name}
              className="overflow-hidden rounded-4xl bg-white"
            >
              <div
              ...
              <div className="p-6 md:p-8">
                ...
                <ul className="mt-5 flex flex-wrap gap-2">
                  {member.roles.map((r) => (
                    <li
                      key={r.org}
                      className="rounded-2xl border border-line bg-paper px-3 py-2 text-xs"
                    >
